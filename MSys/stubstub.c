...
/*
 * This is a copy of tclStubLib.c with a small change to allow implementation of
 * tcl extensions for versions 8 and 9. As long as nobody can tell me why 64 bit
 * versions of tcl are binary incompatible between version 8 and 9, while 32 bit
 * versions remain binary compatible, we can use the following trick in
 * Tcl_InitStubs:
 * If the stub magic check fails, we set it to the magic for Tcl version 9. If it
 * fails again, we reset it to the previous valueand continue processing as usual,
 * inclusive all further checks.
 * Keep in mind: This is only a workaround for 64 bot tcl extensions, for 32 bit 
 * extensions, the stubs magic number of versin 8 and 9 are the same.
 */
    ...
    int magic)
{
    ...
    if ((exact&0xFF00) < 0x900) {
		magic = 0xFCA3BACB + sizeof (void*); /* TCL_STUB_MAGIC from Tcl 9.x */
		if (stubsPtr && magic != stubsPtr->magic)
			magic = 0xFCA3BACF;	/* TCL_STUB_MAGIC from Tcl 8.x */
		tclName = "Tcl";
    }
    /*
